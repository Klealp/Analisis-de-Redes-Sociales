---
title: "Taller 5"
author: "Isabella Agudelo Galindo y Kevin Leal"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
# Introducción
En el presente trabajo se realizará un análisis y comparación de los discursos de posesión presidencial de los últimos 4 presidentes electos en Colombia. Se unifican los dos discursos para los dos períodos de posesión de Álvaro Uribe Vélez (2002-2010), al igual que los dos discursos de posesión de Juan Manuel Santos (2010-2018). Por último se añaden el discurso de Iván Duque Márquez (2018-2022) y el del actual presidente electo Gustavo Francisco Petro Urrego (2022-2026).  
...
Ahora, los textos se almacenan en un objeto tibble, entendido como una generalización del dataframe, el cual nos permitirá manipular de forma más sencilla objetos muy grandes como lo son los discursos. 
```{r}
...
Como segundo paso para el estudio debemos realizar una normalización del texto. Esta empieza con la decisión sobre si se deben conservar los números presentes en los discursos o no. La búsqueda de caractéres numéricos para cada discurso se muestra a continuación.
...
Ahora, se exporta una lista de "stop words" con 451 palabras en español que nos permitirá eliminar todas las palabras que no aportan información relevante para este análisis, como lo son conectores, artículos o verbos auxiliares de uso común. Una vez hecho esto, vemos como se reducen considerablemente la cantidad de registros en los archivos. 
...
Para finalizar con la normalización, se remueven los acentos del idioma español después de haber eliminado las stop words, así se evita remover palabras que sin acento hubieran podido ser eliminadas en el paso anterior.
...
Una vez hecho el tratamiento a estas bases de datos, podemos empezar el análisis con lo básico de la estadística descriptiva. Se realiza un conteo de las palabras y se identifican las modas o las palabras más frecuentes para cada uno de los discursos, A continuación se presentan el top 10 en cada caso.
...
Ahora, para poder realizar comparaciones más directas entre los discursos, se calculan primero las frecuencias relativas de todas las palabras que son mencionadas en ellos, por lo cual, algunas tendrán frecuencia cero si no fueron mencionadas dentro de las alocución del respectivo estadista.
...
```{r, include=FALSE}
library(xtable)
print(xtable(Tabla,align=c("c","c","c","c","c")))
```
...
sentiment_words %>%
  count(sentimiento)
```
Se obtiene el porcentaje de palabras negativas y el de palabras positivas para cada discurso

```{r}
# ---------- petro ----------
text_petro %>%
  inner_join(sentiment_words) %>%
  count(word, sentimiento, sort = TRUE) -> sent_petro

sent_petro %>%
  group_by(sentimiento) %>%
  summarise(conteo = sum(n)/sum(sent_petro$n)*100) -> porc_petro
# ---------- duque ----------
text_duque %>%
  inner_join(sentiment_words) %>%
  count(word, sentimiento, sort = TRUE) -> sent_duque

sent_duque %>%
  group_by(sentimiento) %>%
  summarise(conteo = sum(n)/sum(sent_duque$n)*100) -> porc_duque
# ---------- santos ----------
text_santos %>%
  inner_join(sentiment_words) %>%
  count(word, sentimiento, sort = TRUE) -> sent_santos

sent_santos %>%
  group_by(sentimiento) %>%
  summarise(conteo = sum(n)/sum(sent_santos$n)*100) -> porc_santos
# ---------- uribe ----------
text_uribe %>%
  inner_join(sentiment_words) %>%
  count(word, sentimiento, sort = TRUE) -> sent_uribe

sent_uribe %>%
  group_by(sentimiento) %>%
  summarise(conteo = sum(n)/sum(sent_uribe$n)*100) -> porc_uribe

Porcentajes <- cbind(porc_uribe, porc_santos$conteo, porc_duque$conteo, porc_petro$conteo)
colnames(Porcentajes) <- (c("Sentimiento", "% Uribe", "% Santos", "% Duque", "% Petro"))
Porcentajes
```
Con estos diccionarios podemos crear un gráfico de barras para cada personaje mostrando las palabras positivas y negativas con una frecuencia mayor que 2 en el caso de Petro y Duque, y mayor que 3 para Santos y Uribe.
...
En general, por medio de los diagramas de barras, podemos observar que son más la cantidad de palabras positivas mencionadas en los discursos que las palabras negativas, además de que se mencionan con frecuencias considerablemente mayores. Sin embargo, es resaltar que los expresidentes Iván Duque y Juan Manuel Santos tienden a repetir más las palabras negativas que el presidente Gustavo Petro y el expresidente Álvaro Uribe.

Se encontró que las palabras positivas que comparten más recurrentemente son “paz”, “reforma” y “justicia”. Adicionalmente, hay particularidades interesantes de resaltar, por ejemplo, Petro es el único que hace énfasis en temas ambientales evidenciados en las palabras “tierra”, “ganado”, “sostenible” y, por otro lado, Uribe se destaca por considerar múltiples aspectos éticos y morales como “generosidad”, “libertad”, “confianza”, “equilibrio” e incluso “fe”. 
...
indices_santos <- which(degree(gcc_santos)>3)
palabras_santos <- V(gcc_santos)$name[indices_santos]
lab_santos<-rep(NA,vcount(gcc_santos))
lab_santos[indices_santos]<- palabras_santos
...
indices_uribe <- which(degree(gcc_uribe)>3)
palabras_uribe <- V(gcc_uribe)$name[indices_uribe]
lab_uribe<-rep(NA,vcount(gcc_uribe))
lab_uribe[indices_uribe]<- palabras_uribe

par(mfrow = c(1,2), mar = c(1,1,2,1), mgp = c(1,1,1), cex.main=2)
set.seed(176)
plot(gcc_santos, layout = layout_with_lgl, vertex.color = adjustcolor('#FF6347', 0.1), vertex.frame.color = '#FF6347', vertex.size = strength(gcc_santos),vertex.label=lab_santos, vertex.label.color = 'black', vertex.label.cex = 0.9, vertex.label.dist = 0.5, edge.width = 3*E(g_santos)$weight/max(E(g_santos)$weight),edge.color="#FF6347", main = " Componente de Santos")

plot(gcc_uribe, layout = layout_with_kk, vertex.color = adjustcolor('#FF6347', 0.1), vertex.frame.color = '#FF6347', vertex.size = strength(gcc_uribe),vertex.label=lab_uribe, vertex.label.color = 'black', vertex.label.cex = 0.9, vertex.label.dist = 0.5, edge.width = 3*E(g_uribe)$weight/max(E(g_uribe)$weight),edge.color="#FF6347", main = "Componente de Uribe")
```
...
Una vez más, se decartan todos los registros que contengan alguna stop word, y simultánemente se eliminan los registros que contenían algún número.
...
```{r, echo=F}
V(gcc2_santos)$membership <- kc_santos$membership
